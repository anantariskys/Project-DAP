    ...
    <>
      <nav style={navbarStyle} className={`${navbar?'backdrop-blur-xl ':'backdrop-blur-none'} z-50  fixed flex h-[60px] md:h-[80px] w-full justify-between duration-500  ease-in-out   `}>
        ...
        <div className=" h-full flex flex-row items-center text-white font-bold font-Poppins gap-5 mr-5 md:mr-20 ">
          ...
          <Link to="/"><img src={home} className="h-6" alt="" /></Link>
          <Link to="/auth"><img src={profile} className="h-6 " alt="" /></Link>
          <img src={dashboard2} onClick={hamburgerHandler} className={`h-6 block md:hidden ${hamburger ? "rotate-45" : "rotate-0"}  duration-500`} alt="" />
        </div>
      </nav>
      <div className={` ${hamburger ? "   bg-transparent  backdrop-blur-3xl top-0 mt-20 " : " top-[-500px]  "} fixed  z-50  w-full  md:hidden duration-1000 transition-all ease-in-out`}>
        <div className="flex justify-start  px-8">
          <h3 className="text-lg font-bold font-Poppins text-white">Menu</h3>
        </div>
        <div className="flex flex-col px-8 w-[90%] gap-y-3 mt-4 border-l-[1px] border-b-[1px] border-r-[1px] mx-auto pb-6 rounded-xl ">
